...
# Show a masked version of the DSN so we can eyeball it (password hidden)
masked = DSN.split("@")[-1] if "@" in DSN else "NO @ FOUND"
...
try:
    ...
    st.exception(e)   # prints the full, unredacted error
    st.stop()
...
traffic["ts"] = pd.to_datetime(traffic["event_date"].astype(str)) + pd.to_timedelta(
    traffic["event_hour"], unit="h")
st.plotly_chart(px.line(traffic, x="ts", y="requests"), use_container_width=True)

# --- status codes ---
st.subheader("Response status codes")
status = q("SELECT sc_status, SUM(cnt) AS total FROM gold_status_daily "
           "GROUP BY sc_status ORDER BY total DESC")
st.plotly_chart(px.bar(status, x="sc_status", y="total"), use_container_width=True)

# --- top pages ---
st.subheader("Top pages")
pages = q("SELECT cs_uri_stem, SUM(views) AS views FROM gold_top_pages "
          "GROUP BY cs_uri_stem ORDER BY views DESC LIMIT 20")
st.dataframe(pages, use_container_width=True)

# --- funnel ---
st.subheader("Engagement funnel — top markets")
funnel = q("SELECT funnel_market, SUM(events) AS events, "
           "SUM(unique_visitors) AS visitors FROM gold_funnel "
           "GROUP BY funnel_market ORDER BY events DESC LIMIT 15")
